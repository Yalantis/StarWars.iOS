//
//  StarWarsWindAnimator.swift
//  case_Studies
//
//  Created by Isabel  Lee on 03/05/2017.
//  Copyright © 2017 isabeljlee. All rights reserved.
//

import UIKit

open class StarWarsWindAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    
    open var duration: TimeInterval = 2
    open var spriteWidth: CGFloat = 20
    open var wind: CGFloat = 3
    open var float: CGFloat = 0.1
    ...
    open func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let windPush = UIPushBehavior(items: snapshots, mode: .instantaneous)
        windPush.pushDirection = CGVector(dx: wind, dy: -float)
        windPush.active = true
        self.animator.addBehavior(windPush)
